...
import { FaPlus, FaTrash, FaTruck, FaPhone, FaSearch, FaEdit } from 'react-icons/fa';
import { addSupplier, deleteSupplier, updateSupplier } from '../../../features/inventory/inventorySlice';
...
const Suppliers = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      try {
        ...
        showAlert('error', error || 'Failed to save supplier');
      } finally {
        stopLoading();
      }
    },
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure? This action cannot be undone if stock is linked.')) {
      startLoading();
      try {
        await dispatch(deleteSupplier(id)).unwrap();
        showAlert('success', 'Supplier deleted');
      } catch (error) {
        showAlert('error', 'Cannot delete supplier. It may be in use.');
      } finally {
        stopLoading();
      }
    }
    ...
    <div>
              ...
              <Card className="h-100 border-0 shadow-sm rounded-4 hover-shadow transition-all">
                <Card.Body className="p-4">
                  ...
                  <div className="mt-3 pt-3 border-top">
                    ...
                    <Badge bg="success" className="fw-normal rounded-pill px-3 opacity-75">
                      Verified Supplier
                    </Badge>
                  </div>
                </Card.Body>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        ...
        <Modal.Body className="pt-0 mt-3">
          <p className="text-muted small mb-4">Enter details for the new supplier or distributor.</p>
          <Form onSubmit={formik.handleSubmit}>
            <Form.Group className="mb-3">
              <Form.Label className="small fw-bold text-muted">Supplier Name</Form.Label>
              <Form.Control
                type="text"
                placeholder="e.g. MedLink Distribution"
